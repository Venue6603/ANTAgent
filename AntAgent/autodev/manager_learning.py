    ...
    for lit in lits:
        ...
        for i, ln in enumerate(lines):
            if lit in ln:
                ...
                break
    return anchors[:10]
# --- LLM self-critique loop for robust diffs (general, no prompt-specific hacks) ---
def _llm_generate_validated_diff(
    propose_fn,            # function(enhanced_goal, constraints) -> tuple(summary, diff, explanation) | diff
    enhanced_goal: str,
    constraints: dict,
    validate_fn,           # function(diff_text) -> None or raise Exception(error_msg)
    *,
    max_tries: int = 3
):
    """
    Ask the LLM for a unified diff. If validation fails, feed the exact validator error
    back into the next prompt and ask for a corrected diff. Repeat up to max_tries.
    No deterministic replacements; the LLM must fix its own output.
    """
    last_err = None
    last_summary = ""
    last_explanation = ""
    for attempt in range(1, max_tries + 1):
        # Augment the goal with strict formatting rules and prior validator feedback (if any)
        strict_suffix = (
            "\n\n# RULES\n"
            "- Output ONLY a valid unified diff in 'git diff --no-index' format.\n"
            "- Do NOT include prose, code fences, or any text before/after the diff.\n"
            "- Each hunk must have '@@' lines and at least one change. Add context lines around the change.\n"
            "- Edit ONLY files inside the provided allowed paths/constraints.\n"
        )
        if last_err:
            strict_suffix += f"- Your previous diff was rejected by the validator:\n  {last_err}\n- Correct the diff and try again.\n"

        ask_goal = enhanced_goal + strict_suffix

        res = propose_fn(ask_goal, constraints)
        ...
        if isinstance(res, tuple):
            if len(res) == 1:
                ...
                summary, diff = res[0] or "", res[1] or ""
            else:
                summary, diff, explanation = res[0] or "", res[1] or "", res[2] or ""
        else:
            diff = res or ""

        diff = (diff or "").replace("\r\n", "\n").strip()
        if not diff:
            last_err = "Model produced no diff"
            last_summary, last_explanation = summary, explanation
            continue

        try:
            validate_fn(diff)  # must raise on failure
            return summary, diff, explanation, None  # success (no validator error)
        except Exception as e:
            last_err = str(e)
            last_summary, last_explanation = summary, explanation
            # loop and ask again with the validator error embedded
